...
class AliexpressSpider(RedisSpider):
    ...
    # def parse(self, response):
        ...
        if int(shop_comment_num) > 0:
            ...
            yield ALIItem
            # return self.parse_comment(shop_id, page_num, comment_base_url)
            ...
            comment_le = comment_index_response.xpath("//div[@class='feedback-list-wrap']//div[@class='feedback-item clearfix']")
            for i in comment_le:
            ...
            if page_num >= 1:
                ...
                for i in range(page_num):
                    ...
                    print("开始解析页数不为1的评价")
                    shop_id = shop_id
                    comment_le = new_response.xpath("//div[@class='feedback-list-wrap']//div[@class='feedback-item clearfix']")
                    for i in comment_le:
                        ALIEComment = ALIExpressCommentItem()
                        member_id = i.xpath(".//span[@class='user-name']/a/@href").re_first(
                            "ownerMemberId=(.*)&memberType=.*")
