...
class WencaiSpider(scrapy.Spider):
    ...
    def parse_num_url(self,response):
        link_list = []
        pattern2 = '/stockpick/search.+qs=lm_.+'
        ...
        if links2:
            for link2 in links2:
                ...
                yield SplashRequest(link2.url, endpoint='execute', args={'images': 0, 'lua_source': lua_script},
                                    cache_args=['lua_source'], callback=self.parse_num_url)
        print("开始解析股票页面")
        pattern = '/stockpick/search.+\d{6}$'
        le = LinkExtractor(allow=pattern)
        num_links = le.extract_links(response)
        if num_links:
            self.parse_info_list(num_links)

        # page_num = response.css("span.num::text").re_first("(\d+)")
        # print("获取的页数：%s" % page_num)
        # page_num = int(page_num)
        # # 总条数除以每页显示的数目如果大于一即有下页
        # if page_num % 30 != 0:
        #     page_count = page_num // 30 + 1
        # else:
        #     page_count = page_num // 30
        try:
            page_count = int(response.css("span.total::text").re_first("(\d+)"))
        except Exception as e:
            print("error_page:%s,Error_info:%s" %(response.url,e))
            print("重新下载:%s"%response.url)
    ...
    def parse_info_list(self, num_links):
        for num_link in num_links:
            # link_list.append(num_link.url)
            # print("found url: %s"%num_link.url)
            print("解析到股票详情页：%s"%num_link.url)
            yield SplashRequest(num_link.url, endpoint='execute', args={'images': 0, 'lua_source': lua_script3}, cache_args=['lua_source'], callback=self.parse_info)
    ...
    def parse_info(self, response):
        ...
        try:
            ...
            print("获取到sel")
            pattern3 = '/stockpick/search.+&qs=stockpick_tag$'
            print("匹配到tag结尾的股票列表")
